...
def rerank_results(results, query):
    ...
    def calculate_score(result):
        ...
        occupation = result.get('speaker_occupation', '').lower()  # Add this line
        ...
        occupation_boost = 1.5 if occupation and any(term in occupation for term in query_terms) else 1  # Add this line
        ...
        # Combine all factors
        final_score = term_matches * title_boost * speaker_boost * occupation_boost * view_boost  # Include occupation_boost
...
def hybrid_search(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.info(f"First vector result structure: {str(vector_results[0]) if vector_results else 'None'}")
        ...
        logging.info(f"First SQL result structure: {str(sql_results[0]) if sql_results else 'None'}")
        ...
        for result in vector_results:
            # Convert the complex result to a simple dictionary
            # This is the key fix - we're extracting just the data we need
            simplified_result = {
                "title": result.get("title", "Unknown Title"),
                "speaker": result.get("speaker", "Unknown Speaker"),
                "speaker_occupation": result.get("speaker_occupation", ""),  # Add speaker_occupation
